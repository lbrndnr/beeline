...
//!
//! [`BeelineStream`] takes those frames back out of the stream before the
...
///
/// [`BeelineStream`] fills this in as it comes across sync frames; the
...
impl AsyncRead for BeelineStream {
    fn poll_read(
        self: Pin<&mut Self>,
        cx: &mut Context<'_>,
        buf: &mut ReadBuf<'_>,
...
impl AsyncWrite for BeelineStream {
    fn poll_write(
...
/// updates, see [`BeelineStream`].
pub struct BeelineListener {
    inner: TcpListener,
}

impl BeelineListener {
    ...
    pub async fn accept(&self) -> io::Result<(BeelineStream, SocketAddr)> {
        let (stream, addr) = self.inner.accept().await?;

        let stream = BeelineStream {
            inner: stream,
